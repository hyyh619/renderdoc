...
namespace QtSharedPointer
{
    ...
    struct ExternalRefCountData
    {
        ...
        {
            Q_ASSERT(!weakref.load()); Q_ASSERT(strongref.load() <= 0);
        }
...
public:
    ...
    bool isNull() const Q_DECL_NOTHROW
    {
        return d == Q_NULLPTR || d->strongref.load() == 0 || value == Q_NULLPTR;
    }
    ...
    T* data() const Q_DECL_NOTHROW
    {
        return d == Q_NULLPTR || d->strongref.load() == 0 ? Q_NULLPTR : value;
    }
    ...
    inline QSharedPointer<T> lock() const
    {
        return toStrongRef();
    }

...
struct QHashNode
{
    ...
    inline bool same_key(uint h0, const Key &key0) const
    {
...
public:
...
#endif
    QHash(const QHash &other) : d(other.d)
    {
    ...
    QHash(QHash &&other) Q_DECL_NOTHROW : d(other.d)
    {
        other.d = const_cast<QHashData*>(&QHashData::shared_null);
    }
...
#endif
    void swap(QHash &other) Q_DECL_NOTHROW
    {
        qSwap(d, other.d);
    }
...
public:
    ...
    {
        return erase(const_iterator(it.i));
    }
...
#endif
    void swap(QMultiHash &other) Q_DECL_NOTHROW
    {
        QHash<Key, T>::swap(other);
    }                                                                           // prevent QMultiHash<->QHash swaps
